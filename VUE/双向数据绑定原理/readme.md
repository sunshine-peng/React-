### vue数据双向绑定原理
vue数据双向绑定是通过数据劫持结合发布者-订阅者模式的方式来实现的，通过Object.defineProperty()来实现数据劫持的。对数据的get和set进行重写操作。Object.defineProperty( )对属性设置一个set函数，当数据改变了就会来触发这个函数，所以我们只要将一些需要更新的方法放在这里面就可以实现data更新view了。
...
流程图如下：D:\前端自我学习\VUE\img\双向数据绑定原理流程图.png
1.实现一个Observer
